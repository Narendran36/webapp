pipeline {
  ...
  stages {
    ...
    stage ('DAST') {
      steps {
        sshagent(['zap']) {
          sh 'ssh -o StrictHostKeyChecking=no ubuntu@65.1.111.137 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.110.163.100:8080/webapp/ -x zap-report.xml" || true'
        }
      }
    }
